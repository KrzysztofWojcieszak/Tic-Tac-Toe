...
public class Game {
    ...
    public String checkGameWinner(char [][]grid) {
     ...
     /*   if (((grid[0][0] == grid[0][1]) && (grid[0][1] == grid[0][2])) && (grid[0][1] != '-')) {
            ...
            for (int k = 0; k < 3; k++) {
                if (((grid[k][0] == grid[k][1]) && (grid[k][1] == grid[k][2])) && (grid[k][1] != '-')) {
                    result = grid[k][0] + " WIN";


                } else if (((grid[0][k] == grid[1][k]) && (grid[1][k] == grid[2][k])) && (grid[1][k] != '-')) {
                    result = grid[0][k] + " WIN";

                } else if (((grid[0][0] == grid[1][1] && grid[1][1] == grid[2][2]) || (grid[2][0] == grid[1][1] && grid[1][1] == grid[0][2])) && grid[1][1] != '-') {
                    result = grid[1][1] + " WIN";
                } else if (freeSpots == 0) {
                    result = "TIE";
